...
func (p *Personagem) AumentarNivel(){
	if vitorias >= 5{
		p.Vida += 50
		p.Dano += 20
	}
...
func verificarUpgrade(u Upgrade){
	u.AumentarNivel()
}
...
func vilareijo(p* Personagem, a *Aldeao){
	fmt.Println("Olá caro viajante, meu nome é",a.Nome,"Se estiver a procura de missões eu tenho uma boa pra você:")
